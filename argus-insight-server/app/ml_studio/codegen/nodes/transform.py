"""Code generators for Transform nodes."""
...
def gen_outlier(var: str, parent: str, cfg: dict) -> list[str]:
    ...
    if method == "zscore":
        ...
        lines.append(f'_z = np.abs(stats.zscore({var}[_ocols], nan_policy="omit"))')
        lines.append(f"{var} = {var}[(_z < {threshold}).all(axis=1)]")
    else:
        lines.append("for _c in _ocols:")
        lines.append(f"    _q1, _q3 = {var}[_c].quantile(0.25), {var}[_c].quantile(0.75)")
        lines.append("    _iqr = _q3 - _q1")
        lines.append(f"    {var} = {var}[({var}[_c] >= _q1 - {threshold} * _iqr) & ({var}[_c] <= _q3 + {threshold} * _iqr)]")
...
def gen_scale(var: str, parent: str, cfg: dict) -> list[str]:
    ...
    lines.append(f"{var}[_num] = _scaler.fit_transform({var}[_num])")
