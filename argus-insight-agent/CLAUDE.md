...
| server | port | server.port | 4501 | 바인드 포트 |
...
make run            # uvicorn app.main:app --host 0.0.0.0 --port 4501 --reload
...
# Process
python -m app.processmgr.cli list
python -m app.processmgr.cli list --sort cpu_percent --limit 20
python -m app.processmgr.cli list --user alice
python -m app.processmgr.cli detail 1234
python -m app.processmgr.cli signal 1234 --signal SIGTERM
python -m app.processmgr.cli signal 1234 --signal SIGKILL
python -m app.processmgr.cli restart 1234
python -m app.processmgr.cli zombies
python -m app.processmgr.cli kill-user alice
python -m app.processmgr.cli kill-user alice --signal SIGTERM
python -m app.processmgr.cli too-many-open-files
...
# Process
argus-insight-process list
argus-insight-process list --sort cpu_percent --limit 20
argus-insight-process list --user alice
argus-insight-process detail 1234
argus-insight-process signal 1234 --signal SIGTERM
argus-insight-process restart 1234
...
| `argus-insight-host` | `app.hostmgr.cli` | 호스트 관리 (hostname, /etc/hosts, /etc/resolv.conf, ulimit, limits.conf) |
...
| UI | URL | 설명 |
|----|-----|------|
| Swagger UI | `http://<host>:4501/docs` | 대화형 API 테스트 가능 (Try it out) |
| ReDoc | `http://<host>:4501/redoc` | 읽기 전용 API 문서 |
| OpenAPI JSON | `http://<host>:4501/openapi.json` | OpenAPI 3.x 스펙 (JSON) |

개발 환경 예시:
- Swagger UI: http://localhost:4501/docs
- ReDoc: http://localhost:4501/redoc
...
| GET       | /api/v1/process/list             | 프로세스 목록 (?sort_by=pid&limit=0&username=)          |
| GET       | /api/v1/process/detail/{pid}     | 프로세스 상세 (CPU/메모리/open files/OOM/limits)         |
| POST      | /api/v1/process/signal           | 프로세스 시그널 전송 (SIGTERM/SIGKILL 등)                |
| POST      | /api/v1/process/restart/{pid}    | 프로세스 재시작 (SIGHUP)                                |
| GET       | /api/v1/process/zombies          | 좀비 프로세스 목록                                      |
| POST      | /api/v1/process/kill-user        | 특정 사용자 전체 프로세스 강제 종료                        |
| GET       | /api/v1/process/too-many-open-files | open files 90%+ 프로세스 목록 (?threshold=90)          |
| GET       | /api/v1/process/cpu              | CPU 점유율 기준 프로세스 목록 (?limit=50)                 |
