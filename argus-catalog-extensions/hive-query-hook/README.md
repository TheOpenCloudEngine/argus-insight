...
| 필드 | 타입 | 설명 |
|------|------|------|
| `platformId` | `String` | Argus Catalog의 플랫폼 식별자. `hive-site.xml`의 `QueryAuditHook.platform.id` 값. 카탈로그의 `catalog_platforms.platform_id`와 일치해야 lineage 연결 가능 |
| `inputs` | `List<String>` | 쿼리가 읽는 테이블 목록 (`db.table` 형식). `HookContext.getInputs()`에서 TABLE 타입만 추출 |
| `outputs` | `List<String>` | 쿼리가 쓰는 테이블 목록 (`db.table` 형식). `HookContext.getOutputs()`에서 TABLE 타입만 추출 |
...
<property>
  ...
  <value>http://10.10.10.10:4610/collector/hive/query</value>
</property>

<property>
  <name>QueryAuditHook.platform.id</name>
  <value>hive-019538a3e7c84f2b1</value>
  <description>Argus Catalog에 등록된 플랫폼 ID. catalog_platforms.platform_id와 일치해야 합니다.</description>
</property>
```
...
> **platform.id 확인 방법**: Argus Catalog Server의 `GET /api/v1/catalog/platforms` API를 호출하여 해당 Hive 클러스터의 `platform_id` 값을 확인합니다.
...
* Target URL
  ...
  * Value : `http://10.10.10.10:4610/collector/hive/query`
* Platform ID
  * Name : `QueryAuditHook.platform.id`
  * Value : `hive-019538a3e7c84f2b1`
